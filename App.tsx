...
const App: React.FC = () => {
  ...
  const handleVirtualKeyPress = (keyData: KeyData) => {
    ...
    const charToInsert = keyboardState.isShift 
        ? keyData.kmShift 
        : (keyboardState.isCaps && !['Digit1','Digit2','Digit3','Digit4','Digit5','Digit6','Digit7','Digit8','Digit9','Digit0','Minus','Equal'].includes(keyData.code) 
            ? keyData.kmShift 
            : keyData.km);
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (isKhmerMode && !e.ctrlKey && !e.altKey && !e.metaKey) {
        ...
        if (keyData) {
            const isTypingKey = (!keyData.type || keyData.type === 'char') && (keyData.km || keyData.kmShift);
            ...
            if (isTypingKey) {
                ...
                const char = isShift 
                    ? keyData.kmShift 
                    : (isCaps && !isNumberRow ? keyData.kmShift : keyData.km);
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative overflow-x-hidden">
      ...
      {/* Documents Sidebar */}
      <div className={`fixed top-0 right-0 h-full w-80 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-2xl z-[70] transform transition-transform duration-300 ease-in-out flex flex-col ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                savedDocs.map(doc => (
                    ...
                    >
                        <div className="flex justify-between items-start mb-1">
                            <h3 className="font-semibold text-slate-800 dark:text-slate-100 line-clamp-2 text-sm leading-tight font-khmer">
                                {doc.title}
                            </h3>
                            <button 
      ...
      <main className={`flex-1 w-full mx-auto flex flex-col ${isFullScreen ? '' : 'max-w-5xl p-4 sm:p-6 lg:p-8 gap-6'}`}>
        ...
        {/* Editor Container */}
        <div className={`flex flex-col bg-white dark:bg-slate-800 overflow-hidden transition-all ${
            isFullScreen 
          ...
          <div className="flex flex-wrap items-center gap-0.5 sm:gap-1 p-2 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
            ...
            <ToolbarButton 
                ...
                icon={showInvisibles ? Eye : EyeOff} 
                title={showInvisibles ? "Hide Invisible Characters" : "Show Invisible Characters"}
      ...
      {!isFullScreen && (
        <footer className="py-6 text-center text-slate-400 dark:text-slate-600 text-sm">
          &copy; {new Date().getFullYear()} Khmer Typewriter. All rights reserved.
        </footer>
      )}
    </div>
  );
